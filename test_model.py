...
# -------------load configs file----------------------
# pkl_file = open(configs.model_savepath + "config_train.pk", "rb")
# configs_data = pickle.load(pkl_file)
# pprint.pprint(configs_data.__dict__)
# pkl_file.close()
# del pkl_file
...
for i_file in files[: file_num + 1]:
    # Test model by model and draw pcc, rmse and mae curves
    ...
    (cut_var_pred, cut_var_true, cut_nino_pred, cut_nino_true,) = func_pre(
        ...
        needtauxy=configs.needtauxy
    )
    # -----------截选完整年份计算pcc等
    print("\nnino34的PCC等根据1983.1-2021.12计算\n")
    ...
    # (n_yr,terget_mon,lead_max)改变为(n_yr,start_mon,lead_max)
    ...
    ax2.clabel(
        ...
        colors="k",
        fmt="%.1f",
        # manual=[(7, 4), (7, 4.5), (5, 5.3), (5, 7), (4, 8)],
    )
    ...
    # ax1.set_title(i_file[-14:-4])
    plt.savefig("./model/{}.png".format(i_file[-14:-4]))
    # plt.show()
    print("*************" * 8)
